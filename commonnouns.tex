...
\item SemIndefArt ---
  ...
  \hspace*{3em}\record{
    \mfield{restr}{$Q$}{\textit{Ppty}}\\
    \mfield{scope}{$P|_{\mathfrak{F}(\test{restr})}$}{\textit{Ppty}}\\
    \tfield{e}{exist(restr, scope)}}$\urcorner$
...
`card\_at\_least' and `car\_at\_most'
...
\begin{ex}
  \begin{tabbing}
exactly\_2000(\=$\ulcorner\lambda r$:\smallrecord{\smalltfield{x}{$\mathrm{plurality}(\textit{Rec})$}}
...
\label{ex:2kpassgahm}
\end{ex} 
The first argument to `exactly\_2000' is the pluralized version
