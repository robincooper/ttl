...
\nexteg{b}. @@\todo{Need to include context.}
\begin{ex} 
\begin{subex}
...
puzzle. We made this proposal in previous work
...
to our own.  The puzzle is one that Barbara Partee raised while
...
true at exactly the same possibles worlds and times.)  When we
...
it fell into disuse.  As \cite{Lobnerinprep} points out one objection
...
types of argument roles.  These insights have led him in later work
\citep{Lobner2014,Lobnerinprep} to adopt a frame semantic approach
...
having to name the particular fields involved, we shall introduce an operation which maps a record
type, $T$, and a record, $r$ to the result of specifying $T$ with $r$,
which we will notate as $T\parallel r$.  \nexteg{} provides an abstract example of how it works. 
\begin{ex} 
...
Using this notation we can now rewrite (\ref{ex:temprisefixloc}) as
\nexteg{}.
\begin{ex} 
$\lambda r$:\smallrecord{\smalltfield{fix}{\smallrecord{\smalltfield{loc}{\textit{Loc}}}}\\
                         \smalltfield{scale}{(\textit{AmbTempFrame}
...
may be but it will be sufficient for our current purposes.  We move on
...
\record{\tfield{x}{\textit{Ind}} \\
        ...
        \tfield{e}{at(x, loc)}} 
\end{ex} 
The dependent type for a rise in location event is \nexteg{}.
\begin{ex} 
$\lambda
...
characterize the scale function as returning the height of the
...
\section[Using frames in a compositional semantics for the Partee
  puzzle][Using frames in compositional semantics]{Using frames in a compositional semantics for the Partee
  puzzle}
...
\end{enumerate} 
The last point here requires some explanation. Given that TTR has join
...
witnesses of any finite collection of types.  We cannot express a type
corresponding to an infinite set of types.  Also it does not make
explicit any relationship between the various types in the collection,
in this case that all the types in the collection are function types
whose range type is \textit{RecType} and whose domain type is a
subtype of \textit{Rec} with an `x'-field.  In order to deal with this
kind of case, we will use the same technique as we used for parametric
contents in Chapter~\ref{ch:propnames}.  We will treat properties as a
pair consisting of a type (labelled with `bg') and the function
(labelled with `fg') which we have up to now
been calling a property.  \nexteg{a} is an example of the new kind of
property and \nexteg{b} is the new definition of the type,
...
\item \record{\field{bg}{\smallrecord{\smalltfield{x}{\textit{Ind}}}\\
              \field{fg}{$\lambda
                r$:\smallrecord{\smalltfield{x}{\textit{Ind}}} . \smallrecord{\smalltfield{e}{dog($r$.x)}}}}} 
 
\item \record{\tfield{bg}{\smallrecord{\smalltfield{x}{\textit{Type}}}\\
...
although intuitively obvious.  When we say \textit{the temperature is
...
We introduce first a function `SemDefArt' on the model of `SemIndefArt'
which was defined in Chapter~\ref{ch:gram}, example
(\ref{ex:semindefart}).  Given the new definition of properties as
...
the functions (i.e. `fg').  This is given in \nexteg{}.
\begin{ex} 
$\lambda Q$:\textit{Ppty} . \\
...
However, it is not quite right now that we have
allowed \textit{Ppty} to be polymorphic.  The problem has to do with
the definition of $[\downarrow\cdot]$, that is, property extension.  The definition we gave in
...
\begin{ex} 
$\downP{P}$ = $\{a\mid\exists r[r:P$.bg
$\wedge\ r.\mathrm{x}=a\wedge\down{P.\mathrm{fg}(r)}\not=\emptyset]\}$  
\end{ex} 
...
properties introduced in Chapter~\ref{ch:gram} and Appendix~\ref{app:gramrulesuniv}.
\begin{ex} 
% \begin{subex} 
...
\rotiota-operator.\footnote{An alternative is to maintain the intuition
  ...
  the witnesses be represented as \smallrecord{\field{x}{$r$}} and \smallrecord{\field{x}{$a$}} respectively.} (See \citealp{Elbourne2012} for recent discussion
of the use of this in the semantics of definite
descriptions.)
...
the Russellian treatment of definite descriptions is not quite right
for natural language.  (For a recent detailed discussion of the issues
...
\end{ex} 
Our solution to this is to introduce resource situations
...
\record{\field{bg}{\smallrecord{\smalltfield{x}{\textit{Ind}}}}\\ 
        \field{fg}{$\lambda
  r$:\smallrecord{\smalltfield{x}{\textit{Ind}}}. \smallrecord{\smallrfield{e}{$s$}{dog($r$.x)}}}} 
\end{ex} 
...
\label{ex:dogprime}    
\end{ex} 
General definitions of these abbreviations are given in
Appendix~\ref{app:gramrulesuniv}.
...
(where $P$ is a property) just in case there is exactly one component
of $s$ which has the property $P$.
...
property means that it argument, the
...
different options for the foreground of this parametric content present themselves, as
in \nexteg{}.
\begin{ex} 
\begin{subex} 
 
\item $\lambda
  r$:\smallrecord{\smalltfield{e}{unique(dog$'$)}} . \\
\hspace*{2em}$\lambda P$:\textit{Ppty} .
\record{\tfield{e}{the(dog$'$$\restriction$$r$.e, $P$)}} 
 
\item $\lambda
  r$:\smallrecord{\smalltfield{e}{unique(dog$'$)}} . \\
\hspace*{2em}$\lambda P$:\textit{Ppty} .
\record{\tfield{e}{exist(dog$'$$\restriction$$r$.e, $P$)}}

\item $\lambda
  r$:\smallrecord{\smalltfield{e}{unique(dog$'$)}} . \\
\hspace*{2em}$\lambda P$:\textit{Ppty} .
\record{\tfield{e}{every(dog$'$$\restriction$$r$.e, $P$)}} 
 
\end{subex} 
\label{ex:threethes}   
\end{ex} 
It does not make much difference which of these you choose for the
analysis of singular definite descriptions.  Since we are relating the
...
\end{ex} 
In \cite{Cooper2013b} we chose the option corresponding to (\ref{ex:threethes}c), which offers
...
\subsection{Universal grammar resources} (as in Chapter~\ref{ch:propnames})
...
%       \item[\textnormal{\textit{PPpty}} New!] --- \record{\tfield{bg}{\textit{RecType}} \\
%         \tfield{fg}{(bg$\rightarrow$\textit{Ppty})}}
        
%       \item[\textnormal{\textit{Quant}}] ---
%         (\textit{Ppty}$\rightarrow$\textit{RecType})
        
%       \item[\textnormal{\textit{PQuant}} New!] --- \record{\tfield{bg}{\textit{RecType}} \\
...
\item[Predicates] \mbox{}
  ...
  \item[with arity \textnormal{$\langle\textit{Ind}\rangle$}]
    \{conductor, composer, pianist, leave, hug, dog \textbf{New!}\}
  \end{description}
