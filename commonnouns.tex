...
\item $\ulcorner\lambda
  c$:\smallrecord{
  \footnotesize{\textit{Cntxt}}\\  \smalltfield{$\mathfrak{c}$}{\smallrecord{\smalltfield{e}{unique(dog$'$)}}}} . \\
\hspace*{2em}$\lambda P$:\textit{Ppty} .
\record{
  \mfield{restr}{dog$'$$\restriction$$c.\mathfrak{c}$.e}{\textit{Ppty}}\\
  \mfield{scope}{$P$}{\textit{Ppty}}\\
...
Here we will choose \preveg{c}.  This assumes that we have a predicate
...
\item If $\alpha$ : ($T_1\rightarrow$ \smallrecord{\smalltfield{bg}{\textit{CntxtType}}\\
...
\begin{shaded}\label{pg:fixedpointtype-discussion}
  ...
  \begin{ex}
    $\lambda r$:\smallrecord{
      \smalltfield{x}{\textit{Rec}}} . \record{
      \tfield{x}{\textit{Ind}}\\
      \tfield{c}{$p(\text{x})$}\\
      \tfield{e}{$q(r.\text{x})$}}
...
\begin{ex} 
  \record{
    \tfield{x}{(\textit{Rec}$\wedge$\textit{Ind})}\\
    \tfield{c}{$p(\text{x})$}\\
    \tfield{e}{$q(\text{x})$}}
\end{ex} 
...
\end{ex} 
In the case of \preveg{a} we can construct the fixed point type in
\nexteg{}.
\begin{ex} 
  \record{
    \tfield{x}{\textit{Ind}}\\
...
\smallrecord{\smalltfield{e}{$p(a)$}}.  Now consider
...
\textit{quasi-fixed point}.  Consider the record in \nexteg{a} which
is of the type \nexteg{b}.
\begin{ex} 
\begin{subex} 
...
\smallrecord{\field{e}{$p(\text{\smallrecord{\field{x}{$a$}}})$}}.  We
say that $r$ is a quasi-fixed point for $\mathcal{T}$ and that
\preveg{b} is a quasi-fixed point type for $\mathcal{T}$ as
characterized in \nexteg{}, repeated in the
Appendix on p.~\pageref{pg:fixedpointtype}.
\begin{ex} 
...
We can then define dynamic versions of the contents of quantifier
...
\nexteg{}.
\begin{ex} 
$\ulcorner\lambda c$:\textit{Cntxt} . $\lambda P$:\textit{Ppty} . $P$(\smallrecord{\field{x}{$n$}})$\urcorner$ 
\end{ex} 
...
\begin{ex} 
$\ulcorner\lambda c$:\textit{Cntxt} . $\ulcorner\lambda r$:\smallrecord{\smalltfield{x}{\textit{Ind}}} . 
...
\begin{ex} 
$\ulcorner\lambda c$:\textit{Cntxt} . $\ulcorner\lambda r$:\smallrecord{\smalltfield{x}{\textit{Rec}}} . 
...
\end{ex} 
(We use a conditional in \preveg{b} rather than a biconditional
...
%We first distinguish between singular and plural
...
We introduce a type of plural properties, \textit{PlPpty}, considered
...
Let us take these definitions through our example \textit{2000
  passengers get a hot meal}.  One relevant type which could serve as the
content of this sentence is given in \nexteg{}.
\begin{ex}
  \begin{tabbing}
at\_least\_2000(\=$\ulcorner\lambda r$:\smallrecord{\smalltfield{x}{$\mathrm{plurality}(\textit{Rec})$}}
                                      . \smallrecord{\smalltfield{e}{passenger\_frame\_pl($r$.x)}}$\urcorner$,\\ 
\>$\ulcorner\lambda
r$:\smallrecord{
  \smalltfield{x}{$\mathrm{plurality}(\textit{Rec})$}\\
\smalltfield{e}{passenger\_frame\_pl(x)}} . 
\smallrecord{\smalltfield{e}{get\_a\_hot\_meal\_frame\_pl($r$.x)}}$\urcorner$)
\end{tabbing}
\label{ex:2kpassgahm}
\end{ex} 
% The first argument to `at\_least\_2000' is the pluralized version
...
% (\ref{ex:contgetahotmealframe}).
% \preveg{} makes use of a
...
\item $\mathcal{F}$($\lambda
                                      r$:\smallrecord{\smalltfield{x}{$\mathrm{plurality}(\textit{Rec})$}})
                                      . \\
\hspace*{5em}\smallrecord{\smalltfield{e}{passenger\_frame\_pl($r$.x)}}) 
 
\item
  \record{\tfield{x}{$\mathrm{plurality}(\textit{Rec})$}\\
...
Then we compute the result of restricting the second argument to $s$,
that is,  $Q\!\restriction\!s$.  This is given in \nexteg{}.
\begin{ex} 
$\lambda
               r$:\smallrecord{\smalltfield{x}{$\mathrm{plurality}(\textit{Rec})$}\\
          \smalltfield{e}{passenger\_frame\_pl(x)}} . \\
\hspace*{5em}                 \smallrecord{\smallrfield{e}{$s$}{get\_a\_hot\_meal\_frame\_pl($r$.x)}} 
   
\end{ex} 
We then compute the fixed point type of \preveg{}, given in
\nexteg{a} which is identical with \nexteg{b} and equivalent to \nexteg{c}.
\begin{ex} 
\begin{subex} 
...
\item $\mathcal{F}$($\lambda
          ...
          \smalltfield{e}{passenger\_frame\_pl(x)}} . \\
\hspace*{5em}
\smallrecord{\smallrfield{e}{$s$}{get\_a\_hot\_meal\_frame\_pl($r$.x)}})

\item \smallrecord{\smalltfield{x}{$\mathrm{plurality}(\textit{Rec})$}\\
          \smalltfield{e}{passenger\_frame\_pl(x)$\wedge$(get\_a\_hot\_meal\_frame\_pl(x)$\!\restriction\!s$)}}
 
\item  \smallrecord{\smalltfield{x}{$\mathrm{plurality}(\textit{Rec})$}\\
          \smallrfield{e}{$s$}{passenger\_frame\_pl(x)$\wedge$get\_a\_hot\_meal\_frame\_pl(x)}}
 
\end{subex} 
   
\end{ex}
\begin{shaded}
  In order to understand why \preveg{b} and \preveg{c} are equivalent
...
\item
  \record{\tfield{x}{card(2000)}}
 
\item
  \smallrecord{\smalltfield{x}{$\mathrm{plurality}(\textit{Rec})$}\\
          \smallrfield{e}{$s$}{passenger\_frame\_pl(x)$\wedge$get\_a\_hot\_meal\_frame\_pl(x)}}
 \d{$\wedge$} 
\record{\tfield{x}{card(2000)}}

\item 
 \smallrecord{\smalltfield{x}{$\mathrm{plurality}(\textit{Rec})\wedge$card(2000)}\\
          \smallrfield{e}{$s$}{passenger\_frame\_pl(x)$\wedge$get\_a\_hot\_meal\_frame\_pl(x)}}
\end{subex} 
...
\item[\textnormal{\textit{Cntxt}}] --- \record{
...
\item[\textnormal{IntransVerbIndToFrame} New!] \mbox{}
...
\item[\textnormal{PluralCommonNoun} New!] \mbox{}
