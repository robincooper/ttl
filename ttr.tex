...
\paragraph{Labels occur in dependent fields.} First of all we need to
...
type.  The full official notation for the unflattened record type is:
\begin{quote}
\record{\tfield{x}{\record{\tfield{x}{\textit{Ind}}\\
                           \tfield{e}{$\langle\lambda v$:\textit{Ind}
                             .  dog($v$), $\langle$x.x$\rangle\rangle$}}}}
\end{quote}
This we can now flatten using the same algorithm as we use for records
obtaining:
\begin{quote}
\record{\tfield{x.x}{\textit{Ind}}\\
...
\record{\tfield{x.x}{\textit{Ind}}\\
        \tfield{x.e}{dog(x.x)}}
\end{quote}
Note that a relabelling such as:
\begin{quote}
x.x $\leadsto$ y
\end{quote}
must replace labels in the dependent fields as well as the labels to
the left of the colon.  Thus the relabelled flattened record would be:
\begin{quote}
...
Expanding the relabelled flattened type will result in:
\begin{quote}
\record{\tfield{y}{\textit{Ind}}\\
