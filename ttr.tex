...
consider the following type, presented in official notation.
\begin{quote}
\record{\tfield{x}{\textit{Ind}}\\
...
notation to official notation.   We can do this by carrying out the
following for each occurrence of a label, $\ell$, in the type associated with
an identifier,$\{i\}$, which is referenced elsewhere in the type:
\begin{enumerate} 
...
sequence of paths:
\begin{quote}
replace $\tau$ with $\lambda v_i:T\ .\ \tau[\{i\}:T\leadsto v_i]$
\end{quote}
