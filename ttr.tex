...
argument to `see' in the original unflattened type:
\begin{quote}
$\lambda r$:\smallrecord{\smalltfield{x}{\textit{Ind}}\\
                         \smalltfield{c}{dog(x)}\\
                         \smalltfield{e}{run(x)}} . 
\record{\mfield{s.x}{$r$.x}{\textit{Ind}}\\
        \mfield{s.c}{$r$.c}{dog(s.x)}\\
        \mfield{s.e}{$r$.e}{run(s.x)}\\
        \tfield{e}{see(sam, $r$)}}
\end{quote}
...
occur in the dependent fields of the unflattened type.  Thus the
dependent type which is the flattening may take several arguments.

[How does expansion work.  Have we given up identity between the
original type and the expansion of the flattening of the type?  Not
quite but there may be several flattenings, expansions]
