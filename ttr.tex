...
functions from labels to values.  If $r$ is the record
$\{\langle\ell_1,v_1\rangle,\ldots,\langle\ell_n,v_n\rangle\}$ then we
define $\mathrm{labels}(r)$ to be the set $\{\ell_1,\ldots,\ell_n\}$.
We use $\mathrm{paths}(r)$ to represent the set of paths of $r$, $\mathrm{labels}(\varphi(r))$.
...
We now turn our attention to relabelling.  If $L$ is a set of labels,
...
We are, however, mainly interested in replacing complete paths in a
...
two order pairs whose first member is `x'.  We can read the problem
...
case $\pi'\leadsto\pi$ is in $\eta^-$. If
$\eta$ is a proper relabelling for $r$ then $\eta^-$ is a proper
relabelling for $[r]_\eta$ and $[[r]_\eta]_{\eta^-}=r$.  [Is this
true?]
...
\item
  \record{\tfield{$\ell_1$}{Subst($\alpha$,$v$,$T_1$,$\pi.\ell_1$)} \\
        ...
        if $\phi$ is \record{\tfield{$\ell_1$}{$T_1$} \\
                             \ldots \\ 
                             \tfield{$\ell_n$}{$T_n$}}
