...
$\langle$\textbf{PType}, {\bf Pred}, \textbf{ArgIndices}, {\it
  Arity\/}$\rangle$, $\langle A,F\rangle$$\rangle$ \textit{has
  singleton types} (Chapter~\ref{ch:percint}, p.~\pageref{ex:singleton-types}) if
% \begin{enumerate} 
...
In this section \todo{How do we distinguish between
  records which contain types and the corresponding record type,
  e.g. [x=$T$] and [x:$T$]?} we will define what it means for a system of complex
