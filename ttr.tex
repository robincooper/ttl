...
if it has record types based on $\langle L,
...
of a particular type. Here we will consider how, for a given record
type, $T$, to compute a flattening, $\varphi(T)$, an unflattening,
$\varphi^-(T)$ and an $\eta$-relabelling, $[T]_\eta$.
...
It will be convenient to define the computation of flattenings and
...
Suppose that instead we had wished to compute the flattening of this
type.  We can apply the standard algorithm for records to the unique
identifier notation, that is,
\begin{quote}
$\varphi$(\record{\tfield{id$_0$}{\record{\tfield{x\{0\}}{\textit{Ind}}\\
                                \tfield{e}{dog(\{0\}:\textit{Ind})}}}\\
        \tfield{id$_1$}{\record{\tfield{e}{run(\{0\}:\textit{Ind})}}}})
\end{quote}
This is, again in unique identifier notation:
\begin{quote}
...
\end{quote}
We assume that relabellings apply independently of whether labels are
associated with a unique identifier or not.  Thus for the above type
...
\end{quote}
Applying the standard flattening algorithm to this will result in a
type where the unique identifier `\{1\}' occurs three times:
\begin{quote}
...
@@ [Consider the example where the unique identifier is not final in
the path -- `see' from the beginning of this section.  Need to adjust
$\varphi$ for types, or relabelling? Introduce path notation for types
in place of flattening and replace paths directly not going through
flattening.  Relabelling of paths must respect unique identifiers.]
