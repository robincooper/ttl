...
The notation $[T]_{\pi_1\leadsto\pi_2}$ used here refers to the type
...
\begin{ex} 
Lex($T_{\mathrm{phon}}$, \textit{NP}) \d{$\wedge$}
\smallrecord{\smallmfield{cnt}{SemPron($T_{\mathrm{phon}}$)}{\textit{PQuant}}} 
\end{ex}
...
combination rules if they were addded).  In order to define this we
will is the notation $\pi_i$ to represent a path in which the last
...
We characterize the incrementation of the $\pi$-indexes in a record
...
\begin{ex}
  $\ulcorner\lambda c$:\smallrecord{
    ...
    \smalltfield{$\mathfrak{c}$}{\smallrecord{
          ...
          \smalltfield{c$_1$}{neuter($\Uparrow\!\!\mathfrak{s}$.x$_0$)}}}
}
                                 . 
\record{\tfield{e}{hug($c.\mathfrak{s}$.x$_0$, $c.\mathfrak{s}$.x$_1$)}}$\urcorner$ 
...
\item[\textnormal{$\alpha\text{@}\beta$} New!] \mbox{}\todo{Needs
    revising for unbound pronouns}
