...
\item \record{\field{bg}{\textit{Rec}}\\ 
        \field{fg}{$\lambda r_1$:\textit{Rec}.$\lambda r_2$:\smallrecord{\smalltfield{x}{\textit{Ind}}}.
...
\end{ex}
The foreground of this parametric property will map any context $r_1$ to the function $\lambda r_2$:\smallrecord{\smalltfield{x}{\textit{Ind}}}.
...
\begin{quote}
\record{\field{bg}{\smallrecord{\smalltfield{f}{$\alpha$.bg}\\
                                \smalltfield{a}{$\beta$.bg}}}\\
        \field{fg}{$\lambda r$:\smallrecord{\smalltfield{f}{$\alpha$.bg}\\
                                \smalltfield{a}{$\beta$.bg}} . $\alpha$.fg($r$.f)($\beta$.fg($r$.a))}}
\end{quote}
...
\item \record{\tfield{prev.bg.x}{\textit{Ind}} \\
        \tfield{prev.bg.e}{named(bg.x, ``Sam'')} \\
...
spelled out in \nexteg{}.
\begin{ex}
\record{\field{bg}{\smallrecord{\smalltfield{f}{\smallrecord{\smalltfield{x}{\textit{Ind}}\\
                                               \smalltfield{e}{named(x,
                                                 ``Sam'')}}}\\
                  \smalltfield{a}{\textit{Rec}}}}\\
        \field{fg}{$\lambda
...
\smallrecord{\smalltfield{prev}{$T_{\mathrm{comm}}$}}.  Let
...
Appendix~\ref{app:specrec}) replaces fields in $T$, \smallrecord{\smalltfield{$\ell$}{$T'$}}, such
that $\ell$ is in the
domain of the relabelling, $\eta$, and for which $\eta$ returns a
path, $\pi$,
in $r$ such that $r.\pi:T'$ with a manifest
field \smallrecord{\smallmfield{$\ell$}{$r.\pi$}{$T'$}}. In the type of the updated shared
...
\nexteg{b} and $\eta$ is the relabelling with domain \{f.x,f.e,a\}
with values as defined in \nexteg{c}, then
