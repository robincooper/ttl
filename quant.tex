...
$T_2$ are of type \textit{ContType} and that $@$ is a combination
operation such as @, @@, @$_{\text{wh}_{i,j}}$, then we say $T_1@^{\mathfrak{S}}T_2$
is also a type with the witness condition in \nexteg{}.
\begin{ex} 
If $\alpha:T_1$, $\beta:T_2$ and $\alpha@\beta$ is defined,
then $\alpha@\beta:T_1@^{\mathfrak{S}}T_2$.
Nothing else is a witness for $T_1@^{\mathfrak{S}}T_2$. 
\end{ex} 
What we need then for the type of parametric contents for the combined
constituents is
$\mathfrak{S}(T_1@^{\mathfrak{S}}T_2)$, that
is, the type of the closure of the set of all combinations under
`$\mathrm{store}$' and `$\mathrm{retrieve}$'.  Note that \preveg{} has
the consequence that if `$\alpha@\beta$' is undefined for all
witnesses, $\alpha$ and $\beta$ of $T_1$ and $T_2$ respectively we
will still obtain a result for $T_1@^{\mathfrak{S}}T_2$, allbeit a
...
We can now introduce versions of `ContForwardApp' operations which
