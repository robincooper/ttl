...
\begin{ex} 
If $P_1:\textit{Ppty}$ and $P_2:\textit{Ppty}$ such that
$P_1.\text{bg}=P_2.\text{bg}=$ \smallrecord{
  \smalltfield{x}{$T$}} for some type $T$, then \textit{the conjunction of $P_1$ and
  $P_2$}, $P_1\&P_2$, is
\begin{quote}
...
\begin{ex}
  ...
  If \=$\alpha$ : \smallrecord{
    \smalltfield{bg}{\textit{CntxtType}}\\
    \smalltfield{fg}{(bg$\rightarrow$\textit{Ppty})}},\\
  \>$\beta$ :
  \smallrecord{
    \smalltfield{bg}{\textit{CntxtType}}\\
    \smalltfield{fg}{(bg$\rightarrow$\textit{Ppty})}} and \\
  \>for \=some
  type, $T$, \\
  \>\>for any $c$, $c:\alpha.\text{bg}$ implies $\alpha(c).\text{bg}$ =
  \smallrecord{
    \smalltfield{x}{$T$}} and \\
  \>\>for any $c$, $c:\beta.\text{bg}$ implies
  $\beta(c).\text{bg}$ = \smallrecord{
    \smalltfield{x}{$T$}},\\
