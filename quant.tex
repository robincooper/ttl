...
\end{ex} 
Any rule that forms a boundary for locality will make reference to
$B$.  Thus for example we can formulate the constituent rule that says
that a sentence can consist of a noun phrase followed by a verb phrase
as \nexteg{}.\todo{See (179) on p. 401}
\begin{ex} 
% \textit{S} $\longrightarrow$ \textit{NP VP} $\mid$ $B$(\textit{NP}$'$(\textit{VP}$'$:\textit{Ppty})):\textit{RecType} 
\end{ex} 
