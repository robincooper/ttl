...
(i.e. $X$ is a witness set for $P$) and a function, $f$,  whose domain
...
\begin{ex} 
$\ulcorner\lambda
          r$:\smallrecord{\smalltfield{x}{\textit{Ind}}} . \record{\tfield{e}{dog($r$.x)}}$\urcorner$ 
\end{ex}
...
specific type of property. In general:  if $T$ is a type, then
\textit{Ppty}($T$) is a type; $P$ : \textit{Ppty}($T$) iff
$P$ : \textit{Ppty} and $P$.bg $\sqsubseteq$
\smallrecord{\smalltfield{x}{$T$}}.  [Modify so that doesn't include
the case where $T'\sqsubseteq T$ and the type of the property is \smallrecord{\smalltfield{x}{$T'$}}????]

A property, $P$, can be \textit{pure}, as in \preveg{}, that is,
...
systematically to a correspond property whose background contains just
...
the farmer owns the donkey are such that the farmer likes the
donkey. Thus we need a property of farmers, not of farmer-donkey
...
\item $\lambda r$:\smallrecord{\smalltfield{x}{\textit{Ind}}\\
                             ...
                             . \record{\tfield{e}{like($r$.x, $r$.y)}}
 
\item $\lambda r$:\smallrecord{\smalltfield{x}{\textit{Ind}}} . \record{\tfield{$\mathfrak{c}$}{\smallrecord{\smallmfield{x}{$r$.x}{\textit{Ind}}\\
                               \smalltfield{c$_1$}{farmer(x)}\\
                               \smalltfield{y}{\textit{Ind}}\\
                               \smalltfield{c$_2$}{donkey(y)}\\
                               \smalltfield{e}{own(x, y)}}}\\
        \tfield{e}{\record{\tfield{e}{like($\Uparrow$$\mathfrak{c}$.x,
              $\Uparrow$$\mathfrak{c}$.y)}}}} 
...
and define it as in \nexteg{}.
\begin{ex} 
If $P$ : ($T\rightarrow$\textit{Type}) where $T\sqsubseteq$
\smallrecord{\smalltfield{x}{$T'$}} (that is, $P$ :
\textit{Ppty}($T'$))[???? Not restrictive enough: $T'$ might be a
supertype of the type mentioned in the `x'-field of the property], then
\begin{quote}
$\mathfrak{P}(P)$ is $\lambda r$:\smallrecord{\smalltfield{x}{$T'$}}
. \record{\tfield{$\mathfrak{c}$}{$T\parallel r$}\\
