...
\item If $T\sqsubseteq\textit{Assgnmnt}$ and
  $\ell\in\mathrm{labels}(T)$, then
  \begin{enumerate} 
...
\item If $T\subseteq\textit{PropCntxt}$ and
  $\ell\in\mathrm{labels}(T)$, then
  \begin{enumerate} 
  ...
  \item otherwise, $T\ominus\ell=T-\{\langle\ell,\varphi\rangle\})$,
    where $\langle\ell,\varphi\in T$ 
    
  \end{enumerate} 

\item If $T\sqsubseteq\textit{PropCntxt}$ and
  $\ell.\pi\in\mathrm{tpaths}(T')$ then
  \begin{quote}
    $T\ominus\ell.\pi= T$\fbox{\d{$\wedge$}}\smallrecord{
      \smalltfield{$\ell$}{$\varphi\ominus\pi$}}, where
    $\langle\ell,\varphi\rangle\in T$
  \end{quote}
