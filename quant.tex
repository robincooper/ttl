...
rather than at least one such object.
...
\end{enumerate}  
\end{ex} 
Note that \preveg{} requires $X$ to be identical with $\downP{P}$ and
...
same pattern.
\begin{ex} 
$X:\text{every}^{\text{w}}(P)$ iff $X=\downP{P}$ 
\end{ex} 
...
The witness condition for `most$^{\text{w}}$' is given in \nexteg{}.
Here we are assuming that there is a threshold,
...
more than the quantifier relation and and the first argument property
...
supporters in the stadium cheered.  Rather we would expect the number
