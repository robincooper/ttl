...
syntax.  Now we need to specify the content in the `cnt'-field.  We
...
\hspace*{1em}Lex($T_{\mathrm{phon}}$, \textit{N}) \d{$\wedge$} \smallrecord{\smallmfield{cnt}{SemCommonNoun($p$)}{\textit{Ppty}}} 
\end{ex} 
Note that the type of the content required here is \textit{Ppty}.  In
Chapter~\ref{ch:infex} we defined the content type \textit{Cnt} to be
identical with \textit{RecType}.  Now we have to revise the definition
of \textit{Cnt} to be (\textit{RecType} $\vee$ \textit{Ppty}).  We
...
redefine the type of contents, \textit{Cnt}, as \textit{RecType}
...
\hspace*{1em}Lex($T_{\mathrm{Phon}}$, \textit{NP}) \d{$\wedge$} \smallrecord{\smallmfield{cnt}{SemPropName($a$)}{\textit{Quant}}} 
\end{ex} 
...
a type which should be added to our definition of \textit{Cnt} which
...
\hspace*{1em} Lex($T_{\mathrm{Phon}}$, \textit{Det}) \d{$\wedge$} \smallrecord{\smallmfield{cnt}{SemIndefArt}{(\textit{Ppty}$\rightarrow$\textit{Quant})}} 
\end{ex} 
...
applied to the content of the noun.  This is a case of \textit{content
  forward application}.  We define a function `CntForwardApp', which is
part of the universal resources, as in \nexteg{}.
\begin{ex} 
$\lambda T_1$:\textit{Type} $\lambda T_2$:\textit{Type} . \\
\hspace*{1em}$\lambda
u$:\smallrecord{\smalltfield{cnt}{$(T_2\rightarrow
    T_1)$}}$^{\frown}$\smallrecord{\smalltfield{cnt}{$T_2$}} . \\
\hspace*{2em}\smallrecord{\smallmfield{cnt}{$u$[0].cnt($u$[1].cnt)}{$T_1$}} 
\end{ex} 
...
`CntForwardApp' to add constraints on content to a phrase structure
rule as in the example in \nexteg{}.
\begin{ex} 
\textit{NP} $\longrightarrow$ \textit{Det} \textit{N} \d{\d{$\wedge$}} CntForwardApp(\textit{Ppty},\textit{Quant}) 
\end{ex} 
Recall from (\ref{eg:NPDetN}a) that  \textit{NP} $\longrightarrow$
\textit{Det} \textit{N} is the function
\nexteg{a}. CntForwardApp(\textit{Ppty},\textit{Quant}) is the
...
\item $\lambda
u$:\smallrecord{\smalltfield{cnt}{$(\mathit{Ppty}\rightarrow
    \mathit{Quant})$}}$^{\frown}$\smallrecord{\smalltfield{cnt}{\textit{Ppty}}} . \\
\hspace*{2em}\smallrecord{\smallmfield{cnt}{$u$[0].cnt($u$[1].cnt)}{\textit{Quant}}} 

\item $\lambda u$ : \textit{Det}$^{\frown}$\textit{N} \d{$\wedge$}
  \smallrecord{\smalltfield{s-event}{\smallrecord{\smalltfield{e}{\textit{Phon}}}}}$^+$ \\
\hspace*{4.9em}  \d{$\wedge$} \smallrecord{\smalltfield{cnt}{$(\mathit{Ppty}\rightarrow
    \mathit{Quant})$}}$^{\frown}$\smallrecord{\smalltfield{cnt}{\textit{Ppty}}}
. \\
\hspace*{1em}\textit{NP} \d{$\wedge$}
\smallrecord{\smalltfield{syn}{\smallrecord{\smallmfield{daughters}{$u$}{\textit{Det}$^{\frown}$\textit{N}}}}}
...
\\
\hspace*{2.5em}\d{$\wedge$}
\smallrecord{\smallmfield{cnt}{$u$[0].cnt($u$[1].cnt)}{\textit{Quant}}}
...
\\
\smallmfield{cnt}{syn.daughters[0].cnt(syn.daughters[1].cnt)}{\textit{Quant}}}
...
we use the notation $\down{T}$ to represent $\{a\mid a:T\}$.
...
\nexteg{}.
\begin{ex} 
$e$:exist($P$,$Q$) iff $\downP{P\!\restriction\!
  e}\cap\downP{Q\!\restriction\! e}\not=\emptyset$ 
...
\end{ex} 
In other words, \preveg{a} is a subtype of \preveg{b}.  Consider an
arbitrary record of type \preveg{a} as given in \nexteg{}.
\begin{ex} 
\record{\field{x}{$a$}\\
        \field{c}{$s_1$}\\
        \field{e}{$s_2$}}
...
%            identical with $b$ 
% \end{ex}
We will call (\ref{ex:cnt-is}) `SemBe'.  It will be included among the
...
\textit{V}) \d{$\wedge$} \smallrecord{\smallmfield{cnt}{SemBe}{(\textit{Quant}$\rightarrow$\textit{Ppty})}} 
\end{ex} 
Among the lexical resources for English we have Lex$_{\mathrm{be}}$(``is'').
   
Now let us see what we get when we combine  (\ref{ex:cnt-is}) with the content of
\textit{a conductor}.  This involves applying (\ref{ex:cnt-is}),
...
u$:\textit{V}\d{$\wedge$}\smallrecord{\smalltfield{s-event}{\smallrecord{\smalltfield{e}{``is''}}}}$^{\frown}$\textit{NP}\d{$\wedge$}\smallrecord{\smalltfield{syn}{\smallrecord{\smalltfield{daughters}{\textit{Det}\d{$\wedge$}\smallrecord{\smalltfield{s-event}{\smallrecord{\smalltfield{e}{``a''}}}} \\
                                                                    \hspace*{5em}$^{\frown}$
\textit{N}\d{$\wedge$}\smallrecord{\smalltfield{cnt}{\textit{Ppty}}}}
}}}. \\
\hspace*{1em} \textit{VP}\d{$\wedge$}\smallrecord{\smallmfield{cnt}{$u$[2].syn.daughters[2].cnt}{\textit{Ppty}}} 
\end{ex} 
...
CntForwardApp(\textit{Quant}, \textit{Ppty})  
