...
Note that this means that if $X$ is the labelled set
(\ref{ex:labelled-sets}c) or \preveg{b}, then
...
\begin{ex} 
$r$ is a \textit{record according to a set of labels $\mathcal{L}$, a
  set of distinguished labels, $\mathcal{D}$, and a type
...
\nexteg{}.
\begin{ex}
For $\Gamma$ a type system, $\mathcal{L}$ a set of labels and
$\mathcal{D}$ a set of distinguished labels
\begin{subex} 
...
\hypo{\ell\in\mathcal{L}-(\mathcal{D}\cup\mathrm{labels}(r))}
...
in a record as characterized in \nexteg{}, repeated in
Appendix~\ref{app:rectypes}.\label{pg:record-paths}
\begin{ex} 
...
We will sometimes use `$\mathrm{paths}$' and `$\mathrm{tpaths}$' without the subscript for these
...
and \textbf{RType} $\subseteq$ \textbf{Type} if
\begin{enumerate} 
...
non-record structures such as ptypes.  However, we will make an
...
we introduce meet types in Chapter~\ref{ch:infex}. We will also want
paths to go into dependent fields which always return a record type
such that for any appropriate arguments to the function in the
dependent field the resulting record types will all have same paths.
This will become important when we introduce relabelling for record
types in Chapter~\ref{ch:propnames}. We will define paths for record
types in terms of the paths of the records which are their
witnesses. The relevant definitions are given in \nexteg{}, repeated
in Appendix~\ref{app:rectypes}.
\begin{ex}
  For a record type, $T$, 
\begin{subex} 
...
\item $\pi\in\mathrm{tpaths}_{\text{rectype}}(T)$ iff for any $r:T$, $\pi\in\mathrm{tpaths}_{\text{rec}}(r)$  
 
\end{subex} 
\label{ex:paths-rectype}   
\end{ex}
In terms of our informal proof theoretic notation this can be
expressed as \nexteg{}.
\begin{ex} 
  ...
  \item
    \begin{prooftree}
      \hypo{[r:T]}
      \ellipsis{}{\pi\in\mathrm{paths}_{\text{rec}}(r)}
      \infer1{\pi\in\mathrm{paths}_{\text{rectype}}(T)}
    \end{prooftree}
    
 
\item \begin{prooftree}
      \hypo{[r:T]}
      \ellipsis{}{\pi\in\mathrm{tpaths}_{\text{rec}}(r)}
      \infer1{\pi\in\mathrm{tpaths}_{\text{rectype}}(T)}
    \end{prooftree} 
 
\end{subex} 
  
\end{ex} 
As with $\mathrm{paths}_{\text{rec}}$, we will often suppress the
subscript on $\mathrm{paths}_{\text{rectype}}$ when there is no risk
of confusion.  
...
\mathcal{L},\mathbf{RType}^n\rangle$ and
\begin{enumerate} 
...
types as a collection of systems of complex types in \nexteg{}, repeated in Appendix~\ref{app:modal}.\label{pg:modal-systems}  We use $M$
