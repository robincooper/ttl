...
\hypo{\Gamma,\mathcal{L}-\{\ell\}\vdash r \text{ record}}
...
\hypo{\Gamma,\mathcal{L}\vdash T\in\textbf{Type}}
\hypo{\ell\in\mathcal{L}}
\hypo{\Gamma,\mathcal{L}-\{\ell\}\vdash
...
\item \begin{prooftree}
\hypo{\Gamma,\mathcal{L}\vdash a:T}
\hypo{\Gamma,\mathcal{L}-\{\ell\}\vdash R\in\text{RType}}
\hypo{\Gamma,\mathcal{L}\vdash r:R}
\hypo{\langle\ell,a\rangle\in r}
\infer4{\Gamma,\mathcal{L}\vdash r:R\cup\{\langle\ell,T\rangle\}}
\end{prooftree}
...
record.  \preveg{d} introduces \textit{ERec} as a distinguished record
type and \preveg{e and f} tells us that the empty set is the only
witness for \textit{ERec}.  \preveg{g and h} tell us respectively that
...
the system.  \preveg{j} introduces record types with one
field.$\langle\ell,T\rangle$ and
\preveg{k} tells us that a record containing a field with label $\ell$
and an object of type  $T$ will be a witness
for such a record type. \preveg{l and m} are inductive rule which,
respectively, tell us that you can add a new field,
$\langle\ell,T\rangle$, to a record type, $R$,
provided $\ell$ is not already a label of $R$ and that a record, $r$, is of
the new type just in case $r:R$ and it contains a field
$\langle\ell,a\rangle$ such that $a:T$.

\todo[inline]{$\mathcal{L}-\ell$ doesn't allow for there to be embedded
  $\ell$'s}
