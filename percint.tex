...
\textit{Tree} to $a$.  In terms of modern type theory (as in
...
problems with logical paradoxes.  The development of type theory which
...
to correspond to what \cite{Gibson1986} might call
...
models of the systems we propose as mathematical objects.  This
...
example, by \cite{Martin-Loef1984}) is normally seen as related to
...
semantics starting from \cite{Montague1974}.    

Your theory is not very interesting if it does not
...
individuals, but as an informal notion which can include both objects
...
their own right, not, for example, as collections of objects.
...
\textit{complex} objects composed of other objects in the theory.
Each of these types is associated with a set of objects, that is, the
objects which are of the type, that is the \textit{witnesses} for the type.  Thus if $T$ is a type and $A$ is the
set of objects associated with $T$, then $a$ is of type $T$ (in
symbols, $a:T$) just in case $a\in A$.  We require that any object $a$
...
The definition of a system of basic types is made precise in Appendix~\ref{sec:basic}.   
...
other -- what \cite{Davidson1967} called events and \cite{BarwisePerry1983} called situations.  We
...
sometimes call it a \textit{ptype} to indicate that it is a type whose
...
What kind of entity are predicates?  The notion is made precise in
Appendix~\ref{app:predicates}.  The important thing about predicates
...
set of sequences of types.

Predicates can be considered as functions from sequences of objects
...
we have not made this explicit in Appendix~\ref{app:predicates}.

A system of complex types (made precise in Appendix~\ref{app:comptypes})
...
different types of situations.  There are thus two important functions
...
correspondence will become important below, however.

Kim sees this situation where $a$ (the boy) hugs $b$ (the dog) and
...
the specific individuals $a$ and $b$ as hug($a$,$b$) does.
...
\end{ex} 
In general $\Sigma
x$:$T_1.T_2(x))$ will have as witnesses any ordered pair the first
member of which is a witness for $T_1$ and the second member of which
is a witness for $T_2(x)$.  Thus this type will be non-empty
(``true'') just in case there is something $a$ of type $T_1$ such that
there is something of type $T_2(a)$.  This means that $\Sigma$-types
...
\end{ex} 
We make the notion of record type precise in
Appendix~\ref{app:rectypes}.  Record types consist of sets of fields
such as \smallrecord{\smalltfield{x}{\textit{Ind}}} and
\smallrecord{\smalltfield{c}{hug(x,y)}}.  Fields themselves are pairs
consisting of a \textit{label} such as `x' or `c' in the first
position (before the `:' in our notation) and a type in the second
position. You cannot have more than one field with the same label in a
record type.  The witnesses of record types are \textit{records}.  These
are also sets of fields, but in this case the fields consist of a
label and an object belonging to a type.  A record, $r$, belongs to a record
...
\item \begin{tabular}{lp{.6\textwidth}}\record{\field{x}{$a$} \\
              \field{y}{$b$} \\
              \field{c}{$s$} } &  where $a$:\textit{Ind},
            $b$:\textit{Ind} and $s$:hug($a$,$b$) \end{tabular}
 
\item \begin{tabular}{lp{.6\textwidth}}\record{\field{x}{$d$} \\
              \field{y}{$e$} \\
              \field{c}{$s'$} \\
              \field{z}{$f$} \\
              \field{w}{$g$}} &  where $d$:\textit{Ind},
            $e$:\textit{Ind}, $s'$:hug($d$,$e$) and $f$ and $g$ are
...
(\ref{eg:rectype}a) this means that the type is \textit{dependent} on
what objects you choose for those labels in the object of the record
type.  Thus in \preveg{a} the type of the object labelled `c' is
hug($a$,$b$) whereas in \preveg{b} the type is hug($d$,$e$).
Actually, the notation we are using here for the dependent types is a
convenient simplification of what is needed as we explain in
Appendix~\ref{app:rectypes}. 

Record types and $\Sigma$-types are very similar in an important
respect.  The type (\ref{eg:rectype}a) will be non-empty (``true'')
...
fields which can be used to access ``components'' of what is being
modelled.  This is useful, for example, when we want to analyze
...
Here the labels in record types correspond to roles (frame elements).
...
Another way of approaching these more general types in type theory is
to use \textit{contexts}.  In \nexteg{} we take \textit{True} to be
the type of non-empty types.
\begin{ex} 
\begin{subex} 
 
\item $x$ : \textit{Ind}, $y$ : \textit{Ind} $\vdash$ hug($x$,$y$) : \textit{True}
 
\item $x$ : \textit{Boy}, $y$ : \textit{Dog} $\vdash$ hug($x$,$y$) : \textit{True}
 
\end{subex} 
   
\end{ex} 
\preveg{a,b} mean in a context where $x$ and $y$ are
individuals or a boy and a dog respectively the type hug($x$,$y$) is
non-empty.  This notation is normally taken to mean universal
...
hug($x$,$y$) is non-empty.  We can also think of the contexts as being
objects belonging to types in our type theory.  Records and record
types give us a way of doing this.  Thus, for example,
(\ref{eg:rectype}a) models the type of context which might be
represented as the sequence of parametric type judgements given in
\nexteg{}.
\begin{ex} 
...
technically \nexteg{} is a distinct context from \preveg{} even though
there is an obvious equivalence between them.  
\begin{ex} 
$y$ : \textit{Ind}, $x$ : \textit{Ind}, $c$ : hug($x$,$y$) 
\end{ex} 
...
some new questions.  We said above that the arity of `hug' is
